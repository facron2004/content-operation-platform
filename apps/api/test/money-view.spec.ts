...
describe('money-view (Phase 5 读路径)', () => {
  ...
  describe('toMoneyView 单条记录', () => {
    it('bigint Fen → 字符串 + Display，保留原 Float', () => {
      ...
      expect(out.paidAmount).toBe(39.9); // 原 Float 保留
    });

    it('number Fen（经 BigIntSerializer 转换后）同样字符串化 + Display', () => {
  ...
  describe('applyMoneyView 递归', () => {
    ...
    it('嵌套对象（含 {success,data} 信封）正确增强 data 内实体', () => {
      const payload = {
        success: true,
        data: {
