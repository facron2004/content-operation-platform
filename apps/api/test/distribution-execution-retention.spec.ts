...
describe('DistributionExecutionRetentionJob', () => {
  const prisma = {
    $executeRawUnsafe: vi.fn()
  };
  let job: DistributionExecutionRetentionJob;
  ...
  it('exports retention longer than interactive timeline window', () => {
    ...
    expect(DISTRIBUTION_EXECUTION_RETENTION_DAYS).toBeGreaterThan(90);
  ...
  it('purgeOlderThan issues batched DELETE with createdAt cutoff + LIMIT', async () => {
    ...
    expect(String(sql)).toContain('DELETE FROM "DistributionExecution"');
  ...
  it('cron entry is overlap-safe', async () => {
    ...
    const first = job.purgeExpiredExecutions();
    await job.purgeExpiredExecutions();
    resolveFirst();
    await first;
