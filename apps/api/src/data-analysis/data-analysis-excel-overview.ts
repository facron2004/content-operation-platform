...
export function addOverviewSheet(wb: ExcelJS.Workbook, report: DataAnalysisReport) {
  ...
  sheet.getCell('A2').value =
    `数据区间：${report.date} ~ ${report.endDate}   ｜   共 ${ov.orderCount} 笔支付订单 ｜ ${ov.merchantCount} 商家 ｜ ${ov.salesmanCount} 业务员`;
  sheet.getCell('A2').font = { ...BODY_FONT, color: { argb: 'FF4B5563' } };

  kpiBlock(sheet, 4, 1, '支付订单数', ov.orderCount, 'int');
  ...
  kpiBlock(sheet, 10, 1, '净客单价', ov.avgOrderValue, 'num');
  ...
  const note =
    ...
    '核销额=已核销(verified)订单的 余额+现金（仅 verifyTime 非空的订单计入，按 paidTime 归算）；净客单价=净GMV÷支付订单数；核销率=核销单数÷支付订单数；' +
    '退款率=退款单数÷支付订单数（单数口径，不再用金额）；结算率=核销金额÷毛GMV；' +
