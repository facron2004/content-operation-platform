...
const listKeys = ['list', 'rows', 'records', 'items', 'data', 'page', 'result'];
...
const pick = (row: AnyRecord, keys: string[]) => {
...
const text = (row: AnyRecord, keys: string[], fallback = '') => {
  const value = pick(row, keys);
  return value === undefined ? fallback : String(value).trim();
};

const number = (row: AnyRecord, keys: string[], fallback = 0) => {
  const value = pick(row, keys);
...
const boolean = (row: AnyRecord, keys: string[], fallback = false) => {
  const value = pick(row, keys);
...
const money = (row: AnyRecord, yuanKeys: string[], centKeys: string[], fallback = 0) => {
...
const dateText = (row: AnyRecord, keys: string[], fallback: string) => {
...
const adminFormUrl = (baseUrl: string | undefined, id: string) => {
  const normalized = normalizeJeesiteBaseUrl(baseUrl || 'https://zdm.zhsh1.cn/a');
  return `${normalized}/bargain/bargainCommodity/form?id=${encodeURIComponent(id)}`;
};

export function normalizeJeesiteBaseUrl(rawUrl: string) {
