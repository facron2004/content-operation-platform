...
const DEFAULT_PATH = '/core/corePartnerShop/listData?pageSize=100&pageNo=1';
...
export interface JeeSitePartnerShopPage {
  pageNo: number;
  pageSize: number;
  count: number;
  list: AnyRecord[];
...
export interface JeeSitePartnerShopQuery {
  page: number;
  pageSize: number;
}
...
function requestTimeoutMs(): number {
  const configured = Number(
    process.env.EXTERNAL_PARTNER_SHOP_FETCH_TIMEOUT_MS ?? process.env.EXTERNAL_FETCH_TIMEOUT_MS
  );
...
@Injectable()
export class JeeSitePartnerShopClient {
  private requestQueue: Promise<void> = Promise.resolve();

  constructor(@Inject(AutoLoginService) private readonly autoLoginService: AutoLoginService) {}

  listPartnerShops(query: JeeSitePartnerShopQuery): Promise<JeeSitePartnerShopPage> {
  ...
  private async fetchPage(query: JeeSitePartnerShopQuery): Promise<JeeSitePartnerShopPage> {
    ...
    for (let attempt = 0; attempt <= MAX_RETRIES; attempt += 1) {
      try {
        ...
        if (isRecord(payload) && payload.result === 'login') {
          ...
          throw new ServiceUnavailableException('JeeSite 合作商店铺接口需要重新登录');
        }
        ...
        if (attempt === MAX_RETRIES) {
          ...
          throw new ServiceUnavailableException(
            `JeeSite 合作商店铺接口请求失败: ${describeError(error)}`
          );
        }
    ...
    throw new ServiceUnavailableException('JeeSite 合作商店铺接口请求失败');
  }
  ...
  private buildRequestUrl(baseUrl: string, query: JeeSitePartnerShopQuery): string {
    const configuredPath = process.env.EXTERNAL_PARTNER_SHOPS_PATH?.trim() || DEFAULT_PATH;
    ...
    if (url.hostname !== allowedHost) {
      throw new BadRequestException(
        `EXTERNAL_PARTNER_SHOPS_PATH host ${url.hostname} 不匹配外部数据源 ${allowedHost}`
      );
    }
  ...
  private async fetchWithTimeout(input: string, init: RequestInit): Promise<Response> {
    ...
    const timeout = new Promise<never>((_, reject) => {
      timer = setTimeout(() => {
        controller.abort();
        reject(new Error(`JeeSite 合作商店铺接口请求超时（>${timeoutMs}ms）`));
      }, timeoutMs);
    });
    const request = async () => {
      ...
      if (response.status >= 300 && response.status < 400) {
        ...
        if (redirectUrl.hostname !== new URL(input).hostname) {
          throw new ServiceUnavailableException('JeeSite 合作商店铺接口发生了不安全跳转');
        }
  ...
  private async parseResponse(response: Response): Promise<unknown> {
    if (!response.ok) {
      throw new ServiceUnavailableException(`JeeSite 合作商店铺接口返回 HTTP ${response.status}`);
    }
    let text: string;
    try {
      ...
      if (error instanceof ResponseBodyTooLargeError) {
        throw new ServiceUnavailableException('JeeSite 合作商店铺接口响应过大');
      }
      throw error;
    }
    if ([LOGIN_FORM_HTML_MARKER, ...LOGIN_PAGE_MARKERS].some((marker) => text.includes(marker))) {
      throw new ServiceUnavailableException('JeeSite 合作商店铺接口返回登录页');
    }
    try {
      return JSON.parse(text) as unknown;
    } catch {
      throw new ServiceUnavailableException('JeeSite 合作商店铺接口返回了非 JSON 数据');
    }
  }

  private normalizePage(payload: unknown, query: JeeSitePartnerShopQuery): JeeSitePartnerShopPage {
    ...
    if (!record || !Array.isArray(record.list) || list.length === 0) {
      ...
      throw new ServiceUnavailableException('JeeSite 合作商店铺接口返回结构不完整');
    }
    const count = rowNumber(record, ['count'], Number.NaN);
    if (!Number.isFinite(count) || count < 0) {
      throw new ServiceUnavailableException('JeeSite 合作商店铺接口返回了无效总数');
    }
    return {
