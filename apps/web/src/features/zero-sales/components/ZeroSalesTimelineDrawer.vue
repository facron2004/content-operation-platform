<template>
  <el-drawer
    ...
    :title="drawerTitle"
    size="520px"
    class="zero-sales-timeline-drawer"
    @closed="onClosed"
  >
...
// Residual #234: match API ZeroSalesTimelineQueryDto Min(7) Max(90).
...
const drawerTitle = computed(() => {
  ...
  return name ? `零动销时间线 · ${name}` : '零动销时间线';
});
...
function goAnalysis() {
  ...
  router.push({
    ...
    query: { from: 'zero-sales-timeline' }
  });
}
