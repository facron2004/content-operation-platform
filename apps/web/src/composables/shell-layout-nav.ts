...
export const PROTO_NAV: NavNode[] = [
  {
    ...
    children: [
      ...
      { path: '/tasks', title: '运营任务', icon: 'List' }
    ]
  },
  {
    kind: 'group',
    ...
    children: [
      ...
      { path: '/users/tags', title: '标签管理', icon: 'Checked' },
  ...
  },
  {
    ...
    children: [
      ...
      { path: '/zero-sales', title: '库存预警', icon: 'Warning' }
    ]
  },
  {
    kind: 'group',
    ...
    title: '商家中心',
    icon: 'OfficeBuilding',
    aliases: ['/merchant-applications', '/merchant-heatmap'],
    children: [
      ...
      { path: '/merchant-sales', title: '商家经营分析', icon: 'DataLine' },
  ...
  },
  {
    ...
    title: '营销中心',
    icon: 'Present',
    aliases: ['/attribution', '/campaigns', '/welfare-points', '/generate', '/audit'],
    children: [
      ...
      { path: '/marketing/analytics', title: '营销效果', icon: 'Histogram' }
    ]
  },
  {
    kind: 'group',
    ...
    title: '私域中心',
    icon: 'ChatLineRound',
    aliases: ['/communities', '/community-library'],
    children: [
      ...
      { path: '/private/analytics', title: '私域效果', icon: 'Histogram' }
    ]
  },
  {
    kind: 'group',
  ...
  },
  {
    ...
    title: '平台治理',
    icon: 'SetUp',
    aliases: ['/settings', '/permission-center', '/audit-logs', '/governance/message-templates'],
    children: [
      { path: '/governance/risk', title: '风控中心', icon: 'Warning' },
      { path: '/governance/approvals', title: '审批中心', icon: 'Checked' },
      { path: '/governance/roles', title: '权限管理', icon: 'SetUp' },
...
export function resolveOpenGroupKeys(path: string): string[] {
  ...
  for (const node of PROTO_NAV) {
    ...
    if (
      node.children.some((c) => path === c.path || path.startsWith(c.path + '/')) ||
      node.aliases?.some((alias) => path === alias || path.startsWith(alias + '/'))
    ) {
      keys.push(node.key);
    }
  }
