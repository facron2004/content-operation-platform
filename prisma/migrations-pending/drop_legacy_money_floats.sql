-- ============================================================================
-- Phase 6 硬删除 · 遗留金额 Float 字段下线迁移（已评审，待执行）
-- ============================================================================
-- 作用：删除 10 张表的 44 个遗留金额 Float 字段，仅保留 *Fen 整数分影子列。
--   模型：ContentPackage / SalesSnapshot / CopyPerformance / DailyMetrics /
--         OrderHeader / Member / MerchantDailyMetrics / PackageSalesDaily /
--         MarketingCampaign / TaskPerformanceDaily
--
-- 前置条件（PRD §7.17.1，缺一不可）：
--   1) 已稳定运行至少一个完整结算周期；
--   2) 已对生产数据库做全量备份；
--   3) 本迁移 SQL 已评审通过；
--   4) 已跑 `npm run db:reconcile` 确认金额对账 0 漂移（本次已确认 ✅）。
--
-- 执行方式（需停机或低峰窗口）：
--   a) npx prisma migrate deploy
--      —— 需先把本文件放入 prisma/migrations/<ts>_drop_legacy_money_floats/migration.sql，
--         并同步删除 schema.prisma 中 44 个 Float 字段、重新生成 Prisma Client、
--         改完所有读旧列的 API/脚本代码（见 MR 关联）。
--   b) 或直接用 sqlite3 / prisma db execute 执行本文件。
--
-- 说明：SQLite 不支持 ALTER TABLE DROP COLUMN 的等价原子操作，本迁移采用
--       prisma 标准「建新表 → 拷贝数据 → 删旧表 → 改名」方式，数据零丢失。
-- 可逆性：执行前务必备份；如需回滚，从备份恢复即可（Float 字段随之恢复）。
-- 生成来源：prisma migrate diff --from-schema-datamodel schema.prisma
--                       --to-schema-datamodel <future-without-floats>
-- ============================================================================
