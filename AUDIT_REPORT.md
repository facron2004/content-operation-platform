...
**P0-1: Dataset cache deduplication completely broken**
...
**P0-2: HtmlFetcher 缺少 await 导致 URL 拼接为 `[object Promise]`**
...
**P1-1: AI Config `baseURL` 缺少 SSRF 校验**
...
**P1-2: 过期缓存 Cookie 阻塞环境变量 Cookie 回退**
...
**P1-3: collectInventoryRows 使用朴素 Map 而非 latestSnapshotsByPackage**
...
**P1-6: CONTENT_CACHE_TTL_MS 双重默认值语义冲突**
...
**P1-7: 前端登录跳转使用 `window.location.hash` (history 模式路由无效)**
...
**P2-2: JWT 无 refresh token rotation、无撤销机制**
...
**P2-3: getRecommendations 端点 ~12 个查询参数无 DTO 校验**
...
**P2-9: Raw HTML 缓存占内存 (500 条 * 50-200KB)**
...
**P2-10: AI maxTokens/temperature 无边界校验**
...
**P2-11: buildInventoryFlag 对 0 天未售误判为 unsold_today**
...
|-------|-------|-----------------|
| Phase 1: P0 修复 | P0-1 ~ P0-3 (缓存去重、await 修复、Schema 统一) | ~4-6h |
| Phase 2: P1 修复 | P1-1 ~ P1-8 (SSRF、Cookie 回退、快照 Map、超时、路由修复) | ~6-8h |
| Phase 3: P2 修复 | P2-1 ~ P2-17 (限速、DTO 校验、组件拆分、测试补充) | ~16-20h |
| Phase 4: P3 润色 | P3-1 ~ P3-12 (CSS 清理、类型整理、依赖升级) | ~4-6h |
